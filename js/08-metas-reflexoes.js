...
function openFormMeta(id){
  ...
  document.getElementById('formMeta').addEventListener('submit', e => {
    ...
    const dados = {titulo, categoria: document.getElementById('mt_categoria').value, prazo: document.getElementById('mt_data').value||null, descricao: document.getElementById('mt_descricao').value.trim(), observacoes: document.getElementById('mt_observacoes').value.trim()};
    if (item){
      ...
      const nova = {id: DB.nextId('META','meta'), ...dados, etapas: [], status:'Em andamento', criadoEm: Date.now(), atualizadoEm: Date.now()};
...
function openFormEtapaMeta(metaId){
  ...
  document.getElementById('formEtapa').addEventListener('submit', e => {
    e.preventDefault();
    const titulo = document.getElementById('et_titulo').value.trim(); if (!titulo) return;
    const etapas = [...(meta.etapas||[]), {id:'ET-'+Date.now(), titulo, concluida:false}];
...
function openFormReflexao(id){
  ...
  document.getElementById('formReflexao').addEventListener('submit', e => {
    e.preventDefault();
    const respostas = {}; PERGUNTAS_REFLEXAO.forEach(([k]) => { const v = document.getElementById(`rf_${k}`).value.trim(); if (v) respostas[k] = v; });
    const dados = {data: document.getElementById('rf_data').value || todayISO(), textoLivre: document.getElementById('rf_livre').value.trim(), respostas};
...
function renderReflexoes(){
  ...
  document.getElementById('vazioReflexoes').hidden = lista.length !== 0;
  box.innerHTML = lista.map(r => {
    ...
    return `<article class="activity-card" data-id="${r.id}">
      <div class="activity-main"><div class="activity-title">🧠 Reflexão de ${formatDateBR(r.data)}</div><div class="activity-description">${escapeHTML(resumo.slice(0,140))}${resumo.length>140?'…':''}</div></div>
